...
// 获取数据
    ...
    private int[][] readAllData(String filename) {
        try {
            ...
            int[][] data = new int[this.numchannels][this.len];
    ...
    public int[] readSampleData(int maxDataNum) {
        try {
            if(maxDataNum <= 0) return new int[0];
            ...
            int count = 0;
            int step = len/maxDataNum;
            if(step == 0) {
                step = 1;
                data = new int[len];
            }else{
                data = new int[maxDataNum];
            }
